...
namespace umdgw {
  ...
  void sz_binary_input_service_t::run() {
    ...
    LOGINFO("sz_binary_input_service start success");
  }
  void sz_binary_input_service_t::stop() {
    socket_->close();
    LOGINFO("stop sz_binary_input_service");
  }
  ...
  void sz_binary_input_service_t::handleRead(int errorcode,std::string detail,int bytes_transferred) {
    if (errorcode) {
  ...
  void sz_binary_input_service_t::handleWrite(int errorcode, std::string detail, int bytes_transferred) {
    if (!errorcode) {
    }
    else {
      doCancelTimer(heartBeatTimer_);
  ...
  void sz_binary_input_service_t::handlemessage_t(uint8_t * content, int size){
    ...
    switch (msgType) {
    case 1://logon msg
      LOGINFO("logon success");
      doSetTimer(heartBeatTimer_, 3000);
      break;
    case 2://logout msg
      LOGINFO("logout received");
      doCancelTimer(heartBeatTimer_);
      doSetTimer(reconnectTimer_, 3000);
      break;
    case 3://heartbeat
