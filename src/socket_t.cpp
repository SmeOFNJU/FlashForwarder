#include "socket.hpp"
namespace umdgw {
  socket_t::socket_t(boost::shared_ptr<service_t> service):
  ...
  void socket_t::close() {
    io_service_ptr_->post(boost::bind(&socket_t::doClose, shared_from_this()));
  }

  void socket_t::doClose() {
    if (type_ == STREAM) {
      if (service_ptr_->getIoType() == SIN) {
        boost::system::error_code ec;
  ...
  boost::shared_ptr<boost::asio::io_service> socket_t::getIOService()
  {
